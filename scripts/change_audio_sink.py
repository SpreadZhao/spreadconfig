...
for item in sinks:
    if item['sink_name'].endswith(" - Default"):
        output += f"<b>-> {item['sink_id']}: {item['sink_name']}</b>\n"
    else:
...
selected_sink_id = int(selected_sink_name.strip('<b>').strip('</b>').split(":")[0].replace("->", "").strip())
sinks = parse_wpctl_status()
selected_sink = next(sink for sink in sinks if sink['sink_id'] == selected_sink_id)
